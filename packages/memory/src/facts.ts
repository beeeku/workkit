...
import type { Fact, FactMetadata, MemoryResult, MemoryError } from "./types";
...
export function createFactStore(db: D1Database) {
  ...
  return {
    async remember(text: string, metadata?: FactMetadata): Promise<MemoryResult<Fact>> {
      try {
        ...
        await db.prepare(
          ...
          fact.id, fact.text, fact.subject, fact.source,
          JSON.stringify(fact.tags), fact.confidence, fact.encrypted ? 1 : 0,
          fact.createdAt, fact.validFrom, fact.validUntil,
          fact.supersededBy, fact.forgottenAt, fact.forgottenReason,
          fact.embeddingStatus, fact.ttl, metadata?.idempotencyKey ?? null,
        ).run();

        // Handle supersession
        if (metadata?.supersedes) {
          await db.prepare(
            "UPDATE facts SET superseded_by = ?, valid_until = ? WHERE id = ?"
          ).bind(fact.id, now, metadata.supersedes).run();
        }

        return { ok: true, value: fact };
      } catch (error: any) {
        if (error.message?.includes("UNIQUE constraint")) {
          return { ok: false, error: { code: "IDEMPOTENCY_ERROR", message: "Fact with this idempotency key already exists" } };
        }
        return { ok: false, error: { code: "STORAGE_ERROR", message: error.message } };
      }
    },

    async rememberBatch(facts: Array<{ fact: string; metadata?: FactMetadata }>): Promise<MemoryResult<Fact[]>> {
    ...
    async forget(factId: string, reason?: string): Promise<MemoryResult<void>> {
      try {
        await db.prepare(
          "UPDATE facts SET forgotten_at = ?, forgotten_reason = ? WHERE id = ?"
        ).bind(Date.now(), reason ?? null, factId).run();
    ...
    async supersede(oldFactId: string, newText: string, metadata?: FactMetadata): Promise<MemoryResult<Fact>> {
      try {
        ...
        const inheritedMetadata: FactMetadata = {
          ...
          tags: metadata?.tags ?? ((oldRow as any)?.tags ? JSON.parse((oldRow as any).tags) : undefined),
