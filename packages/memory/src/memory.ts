...
export function createMemory(options: MemoryOptions): Memory {
	...
	const memory: Memory = {
		...
		async rememberBatch(
			items: Array<{ fact: string; metadata?: FactMetadata }>,
		...
		async compact(_opts?: CompactOptions): Promise<MemoryResult<CompactResult>> {
		...
		async reembed(opts?: ReembedOptions): Promise<MemoryResult<ReembedResult>> {
			if (!embeddings.enabled) {
				return {
					ok: false,
					error: { code: "EMBEDDING_ERROR", message: "No AI binding configured" },
				};
			}
			try {
				const start = Date.now();
				...
				return {
					ok: true,
					value: { processedCount: count, failedCount: 0, durationMs: Date.now() - start },
				};
			} catch (error: any) {
