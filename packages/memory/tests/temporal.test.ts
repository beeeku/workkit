...
describe("createScopedMemory", () => {
	...
	describe("recall", () => {
		it("delegates to deps.recall with timeRange.to set to timestamp", async () => {
			...
			expect(deps.recall).toHaveBeenCalledWith(
				"user preferences",
		...
		it("propagates error from deps.recall", async () => {
			const deps = makeDeps({
				recall: vi.fn(async () => ({
					ok: false as const,
					error: { code: "STORAGE_ERROR" as const, message: "fail" },
				})),
			});
			const scoped = createScopedMemory(TIMESTAMP, deps);
	...
	describe("search", () => {
		...
		it("propagates error from deps.search", async () => {
			const deps = makeDeps({
				search: vi.fn(async () => ({
					ok: false as const,
					error: { code: "STORAGE_ERROR" as const, message: "oops" },
				})),
			});
			const scoped = createScopedMemory(TIMESTAMP, deps);
	...
	describe("get", () => {
		...
		it("propagates error from deps.get", async () => {
			const deps = makeDeps({
				get: vi.fn(async () => ({
					ok: false as const,
					error: { code: "STORAGE_ERROR" as const, message: "db error" },
				})),
			});
			const scoped = createScopedMemory(TIMESTAMP, deps);
			const result = await scoped.get("fact_1");
	...
	describe("stats", () => {
		...
		it("propagates error from deps.stats", async () => {
			const deps = makeDeps({
				stats: vi.fn(async () => ({
					ok: false as const,
					error: { code: "STORAGE_ERROR" as const, message: "stats fail" },
				})),
			});
			const scoped = createScopedMemory(TIMESTAMP, deps);
			const result = await scoped.stats();
