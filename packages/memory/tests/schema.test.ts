...
describe("getSchema", () => {
  ...
  it("has index on facts(valid_from)", () => {
    expect(getSchema()).toContain("CREATE INDEX IF NOT EXISTS idx_facts_valid_from ON facts(valid_from)");
  });

  it("has index on facts(forgotten_at)", () => {
    expect(getSchema()).toContain("CREATE INDEX IF NOT EXISTS idx_facts_forgotten ON facts(forgotten_at)");
  });

  it("has index on facts(embedding_status)", () => {
    expect(getSchema()).toContain("CREATE INDEX IF NOT EXISTS idx_facts_embedding_status ON facts(embedding_status)");
  });

  it("has index on messages(conversation_id, created_at)", () => {
    expect(getSchema()).toContain("CREATE INDEX IF NOT EXISTS idx_messages_conversation ON messages(conversation_id, created_at)");
  });

  it("has index on summaries(conversation_id)", () => {
    expect(getSchema()).toContain("CREATE INDEX IF NOT EXISTS idx_summaries_conversation ON summaries(conversation_id)");
  });
