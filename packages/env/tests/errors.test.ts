...
describe("EnvValidationError", () => {
	...
	it("truncates long string values in error output", () => {
		...
		const issues: EnvIssue[] = [{ key: "API_KEY", message: "Invalid format", received: longValue }];
		const err = new EnvValidationError(issues);
	...
	it('uses singular "issue" for single issue', () => {
		const issues: EnvIssue[] = [{ key: "A", message: "bad", received: undefined }];
		const err = new EnvValidationError(issues);
		expect(err.message).toContain("1 issue found");
	});

	it("provides programmatic access via .issues property", () => {
		const issues: EnvIssue[] = [{ key: "DB", message: "Missing binding", received: undefined }];
		const err = new EnvValidationError(issues);
	...
	it("formats null received value", () => {
		const issues: EnvIssue[] = [{ key: "X", message: "Expected string", received: null }];
		const err = new EnvValidationError(issues);
		expect(err.message).toContain("received: null");
	});

	it("formats object received value", () => {
		const issues: EnvIssue[] = [{ key: "X", message: "Expected string", received: { foo: "bar" } }];
		const err = new EnvValidationError(issues);
