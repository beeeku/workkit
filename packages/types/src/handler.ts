...
export interface WorkerFetchHandler<E = unknown> {
	(request: Request, env: E, ctx: ExecutionContext): Response | Promise<Response>
}

/** A Workers scheduled handler */
export interface WorkerScheduledHandler<E = unknown> {
	(event: ScheduledEvent, env: E, ctx: ExecutionContext): void | Promise<void>
}

/** A Workers queue handler */
export interface WorkerQueueHandler<E = unknown, Body = unknown> {
	(batch: TypedMessageBatch<Body>, env: E, ctx: ExecutionContext): void | Promise<void>
}

/** A Workers email handler */
export interface WorkerEmailHandler<E = unknown> {
	(message: EmailMessage, env: E, ctx: ExecutionContext): void | Promise<void>
}
