...
describe("TypedQueueProducer type", () => {
	it("send() accepts the generic body type", () => {
		expectTypeOf<TypedQueueProducer<UserEvent>["send"]>().parameter(0).toEqualTypeOf<UserEvent>();
	});
	...
	it("raw property exposes the underlying queue", () => {
		expectTypeOf<TypedQueueProducer<UserEvent>["raw"]>().toBeObject();
	});
});
...
describe("ConsumerOptions type", () => {
	...
	it("filter function receives typed message", () => {
		type Opts = ConsumerOptions<UserEvent>;
		expectTypeOf<NonNullable<Opts["filter"]>>().toBeFunction();
	});

	it("maxRetries is optional number", () => {
		expectTypeOf<ConsumerOptions<UserEvent>["maxRetries"]>().toEqualTypeOf<number | undefined>();
	});

	it("onFiltered is optional ack or retry", () => {
		expectTypeOf<ConsumerOptions<UserEvent>["onFiltered"]>().toEqualTypeOf<
			"ack" | "retry" | undefined
		>();
	});

	it("concurrency is optional number", () => {
		expectTypeOf<ConsumerOptions<UserEvent>["concurrency"]>().toEqualTypeOf<number | undefined>();
	});
});
...
describe("BatchConsumerOptions type", () => {
	it("processBatch function receives typed messages array", () => {
		...
		expectTypeOf<Opts["processBatch"]>().toBeFunction();
	});

	it("retryAll is optional boolean", () => {
		expectTypeOf<BatchConsumerOptions<UserEvent>["retryAll"]>().toEqualTypeOf<
			boolean | undefined
		>();
	});
});
...
describe("ConsumerHandler type", () => {
	it("is a function that accepts batch and env", () => {
		expectTypeOf<ConsumerHandler<UserEvent>>().toBeFunction();
	});

	it("returns a promise", () => {
		expectTypeOf<ConsumerHandler<UserEvent>>().returns.toEqualTypeOf<Promise<void>>();
	});
});
