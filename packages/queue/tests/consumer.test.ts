...
import { createBatchConsumer, createConsumer } from "../src/consumer";
...
describe("createConsumer()", () => {
	...
	it("respects maxRetries and acks after exceeding", async () => {
		...
		const msg = createMockMessage<UserEvent>({ type: "created", userId: "1" }, { attempts: 4 });
	...
	it("retries when under maxRetries", async () => {
		...
		const msg = createMockMessage<UserEvent>({ type: "created", userId: "1" }, { attempts: 2 });
		const batch = createMockBatch("test-queue", [msg]);
	...
	it("sends to dead letter queue after maxRetries", async () => {
		...
		const msg = createMockMessage<UserEvent>({ type: "created", userId: "1" }, { attempts: 3 });
		const batch = createMockBatch("test-queue", [msg]);
...
describe("createConsumer() — filtering", () => {
	...
	it("acks filtered-out messages by default", async () => {
		...
		const messages = [createMockMessage<UserEvent>({ type: "updated", userId: "1" })];
		const batch = createMockBatch("test-queue", messages);
...
describe("createBatchConsumer()", () => {
	...
	it("retries all on failure by default", async () => {
		...
		const messages = [createMockMessage<UserEvent>({ type: "created", userId: "1" })];
		const batch = createMockBatch("test-queue", messages);
