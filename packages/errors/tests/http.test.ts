...
import { ForbiddenError, UnauthorizedError } from "../src/categories/auth";
...
import { errorToResponse, fromHttpStatus, isErrorCode, isWorkkitError } from "../src/http";
...
describe("errorToResponse", () => {
	...
	it("includes validation issues", async () => {
		const error = new ValidationError("Invalid", [{ path: ["name"], message: "Required" }]);
		const res = errorToResponse(error);
		const body = (await res.json()) as any;
