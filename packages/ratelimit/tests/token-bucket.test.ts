...
describe('tokenBucket', () => {
  ...
  it('allows consuming a single token', async () => {
    const limiter = tokenBucket({ namespace: kv, capacity: 10, refillRate: 1, refillInterval: '1s' })
  ...
  it('blocks when not enough tokens', async () => {
    const limiter = tokenBucket({ namespace: kv, capacity: 5, refillRate: 1, refillInterval: '1s' })
  ...
  it('handles refill rate greater than 1', async () => {
    const limiter = tokenBucket({ namespace: kv, capacity: 100, refillRate: 10, refillInterval: '1s' })
  ...
  it('handles capacity of 1', async () => {
    const limiter = tokenBucket({ namespace: kv, capacity: 1, refillRate: 1, refillInterval: '1s' })
  ...
  it('remaining never goes below zero', async () => {
    const limiter = tokenBucket({ namespace: kv, capacity: 3, refillRate: 1, refillInterval: '1s' })
