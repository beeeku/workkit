...
import type { ToolRegistry, ResourceRegistry, PromptRegistry } from "./registry";
import { schemaToJsonSchema, validateInput } from "./validation";
import { MCPProtocolError, toMCPToolError, toJsonRpcError } from "./errors";
...
export function createProtocolHandler(config: ProtocolHandlerConfig): ProtocolHandler {
  ...
  async function dispatch(
    ...
    try {
      ...
      return errorResponse(id, MCPProtocolError.internalError(
  ...
  async function handleToolsCall(
    ...
    const ctx = context?.ctx ?? { waitUntil: () => {}, passThroughOnException: () => {} } as ExecutionContext;
  ...
  function handleResourcesList(id: string | number): JsonRpcResponse {
    const resourceList = resources.all()
    ...
    const promptList = prompts.all().map((p) => {
      ...
      if (p.args) {
        ...
        if (schema.properties) {
          item.arguments = Object.entries(schema.properties as Record<string, Record<string, unknown>>).map(
            ([name, prop]) => ({
              name,
  ...
  async function handlePromptsGet(
    ...
    const result = await prompt.handler({
      args: prompt.args ? (await validateInput(prompt.args, args)).value : undefined,
      env,
      ctx,
      log,
    } as any);
