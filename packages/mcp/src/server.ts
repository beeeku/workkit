...
import { createToolRegistry, createResourceRegistry, createPromptRegistry } from "./registry";
...
export function createMCPServer<TEnv = unknown>(
  config: MCPServerConfig<TEnv>,
): MCPServer<TEnv> {
  ...
  function buildHonoApp(): Hono<{ Bindings: TEnv }> {
    ...
    app.post(`${basePath}/tools/:toolName`, async (c) => {
      ...
      const response = await rest.handleRequest(c.req.raw, env, ctx);
  ...
  const server: MCPServer<TEnv> = {
    ...
    serve(): WorkerModule<TEnv> {
      if (toolRegistry.size === 0) {
        console.warn("[workkit/mcp] Server has no tools registered — clients won't discover any capabilities");
      }
