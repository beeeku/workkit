...
describe("buildCacheUrl", () => {
	...
	it("should use custom baseUrl", () => {
		expect(buildCacheUrl("/test", "https://my-app.workers.dev")).toBe(
			"https://my-app.workers.dev/test",
		);
	});

	it("should handle keys with query strings", () => {
		expect(buildCacheUrl("/api/users?page=1", baseUrl)).toBe(
			"https://cache.local/api/users?page=1",
		);
	});

	it("should handle keys with hash fragments", () => {
		expect(buildCacheUrl("/api/data#section", baseUrl)).toBe(
			"https://cache.local/api/data#section",
		);
	});

	it("should handle deep paths", () => {
		expect(buildCacheUrl("/api/v2/users/123/posts", baseUrl)).toBe(
			"https://cache.local/api/v2/users/123/posts",
		);
	});
});
