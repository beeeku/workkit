...
// ─── Approval States ──────────────────────────────────────────
export type ApprovalState = "pending" | "approved" | "denied" | "escalated" | "timed_out" | "cancelled";
...
export type PolicyMatcher = TagMatcher | CostMatcher | RiskMatcher | NameMatcher | CustomMatcher | CompositeMatcher;

// ─── Approver Spec ────────────────────────────────────────────
export type ApproverResolver = (action: ActionDescriptor, context: GuardContext) => Promise<string[]>;
...
export interface PolicyDefinition {
  ...
  validateApproval?: (decision: ApprovalDecision, request: ApprovalRequestData) => { ok: true } | { ok: false; error: string };
  approvalTTL?: string;
}
...
export interface ApprovalGateConfig {
  ...
  signingKey: { privateKey: string; publicKey: string } | { privateKey: CryptoKey; publicKey: CryptoKey };
...
export interface ChannelAdapter {
  name: string;
  send(notification: NotificationPayload): Promise<{ ok: true } | { ok: false; error: ChannelErrorInfo }>;
}
