import { generateSigningKeyPair, sign, randomUUID } from "@workkit/crypto";
...
export async function verifyApprovalToken(
  ...
  if (parts.length !== 2) {
    return { ok: false, error: { code: "MALFORMED_TOKEN", message: "Token must have exactly two parts separated by '.'" } };
  }
  ...
  try {
    ...
    return { ok: false, error: { code: "MALFORMED_TOKEN", message: "Failed to decode token payload" } };
  }

  if (!payload || typeof payload !== "object" || !payload.tid) {
    return { ok: false, error: { code: "MALFORMED_TOKEN", message: "Invalid token payload structure" } };
  }

  // 2. Verify signature
  const isValid = await sign.verify(publicKey, JSON.stringify(payload), signature);
  if (!isValid) {
    return { ok: false, error: { code: "INVALID_SIGNATURE", message: "Token signature verification failed" } };
  }
  ...
  if (payload.rid !== expectedRequestId) {
    return { ok: false, error: { code: "REQUEST_MISMATCH", message: `Token is for request '${payload.rid}', not '${expectedRequestId}'` } };
  }

  // 5. Check approver
  if (payload.sub !== expectedApproverId) {
    return { ok: false, error: { code: "APPROVER_MISMATCH", message: `Token is for approver '${payload.sub}', not '${expectedApproverId}'` } };
  }

  // 6. Check consumed
  if (consumedTokens.has(payload.tid)) {
    return { ok: false, error: { code: "TOKEN_ALREADY_USED", message: "Token has already been consumed" } };
  }
