...
export function matchesPolicy(action: ActionDescriptor, matcher: PolicyMatcher): boolean {
	switch (matcher.type) {
		case "tag": {
			...
			if (matcher.noneOf?.some((t) => tags.includes(t))) return false;
			return true;
		}
		...
		case "risk": {
			...
			return (RISK_ORDER[action.risk] ?? 0) >= (RISK_ORDER[matcher.minLevel] ?? 0);
		}
...
function mergeMatchingPolicies(matches: NamedPolicy[]): ResolvedPolicy {
	...
	const requiredApprovals = Math.max(...matches.map((m) => m.def.requiredApprovals ?? 1));

	const timeoutMs = Math.min(
		...matches.map((m) => (m.def.timeout ? parseDuration(m.def.timeout) : DEFAULT_TIMEOUT_MS)),
	);
	...
	const segregateRequester = matches.some((m) => m.def.segregateRequester !== false);
	...
	const escalation: string[][] = escalationSpec.map((spec) => literalApprovers(spec));
