import type { ChannelAdapter, NotificationPayload, ChannelErrorInfo } from "../types";
...
export function createWebhookChannel(config: WebhookChannelConfig): ChannelAdapter {
  return {
    name: "webhook",
    async send(notification: NotificationPayload): Promise<{ ok: true } | { ok: false; error: ChannelErrorInfo }> {
      try {
