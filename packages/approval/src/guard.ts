...
import type { ActionDescriptor, GuardContext, GuardResult, PolicyDefinition, ResolvedPolicy } from "./types";
...
interface GuardConfig {
  ...
  generateToken: (requestId: string, approverId: string, action: "both", expiresIn: number) => Promise<{ token: string; tokenId: string }>;
  baseUrl?: string;
}

export function createGuard(config: GuardConfig) {
  return async function guard(action: ActionDescriptor, context: GuardContext): Promise<GuardResult> {
    // 1. Evaluate policies
    ...
    await stub.fetch(new Request("https://internal/create", {
      method: "POST",
      body: JSON.stringify({
        id: requestId,
        action,
