...
import { generateApprovalToken, verifyApprovalToken, generateApprovalKeys } from "../src/token";
...
describe("generateApprovalToken", () => {
  it("generates a token with correct structure", async () => {
    ...
    const { token, tokenId } = await generateApprovalToken(
      "req_123", "user_bob", "both", 3600000, keys.privateKey,
    );
...
describe("verifyApprovalToken", () => {
  it("verifies a valid token", async () => {
    const keys = await generateApprovalKeys();
    const { token } = await generateApprovalToken("req_123", "bob", "approve", 3600000, keys.privateKey);
  ...
  it("rejects expired token", async () => {
    ...
    const { token } = await generateApprovalToken("req_123", "bob", "approve", -1000, keys.privateKey);
  ...
  it("rejects consumed token", async () => {
    const keys = await generateApprovalKeys();
    const { token, tokenId } = await generateApprovalToken("req_123", "bob", "approve", 3600000, keys.privateKey);
  ...
  it("rejects malformed token", async () => {
    const keys = await generateApprovalKeys();
    const result = await verifyApprovalToken("not-a-token", "req_123", "bob", keys.publicKey, new Set());
    expect(result.ok).toBe(false);
  });
  ...
  it("rejects token with wrong signature", async () => {
    ...
    const { token } = await generateApprovalToken("req_123", "bob", "approve", 3600000, keys1.privateKey);
