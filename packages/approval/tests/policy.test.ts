...
import { matchesPolicy, evaluatePolicies, globMatch } from "../src/policy";
...
describe("matchesPolicy", () => {
  ...
  it("CostMatcher: currency filter", () => {
    expect(matchesPolicy(action, { type: "cost", greaterThanOrEqual: 1000, currency: "USD" })).toBe(true);
    expect(matchesPolicy(action, { type: "cost", greaterThanOrEqual: 1000, currency: "EUR" })).toBe(false);
  });
  ...
  it("CustomMatcher: predicate", () => {
    expect(matchesPolicy(action, { type: "custom", fn: (a) => a.requestedBy === "alice" })).toBe(true);
    expect(matchesPolicy(action, { type: "custom", fn: (a) => a.requestedBy === "bob" })).toBe(false);
  });

  it("CompositeMatcher: all", () => {
    expect(matchesPolicy(action, {
      type: "all",
      matchers: [
...
describe("evaluatePolicies", () => {
  ...
  it("returns null when no policies match", () => {
    const policies = new Map<string, PolicyDefinition>([
      ["staging-only", { match: { type: "tag", allOf: ["staging"] }, approvers: ["bob"], requiredApprovals: 1, timeout: "1h" }],
    ]);
