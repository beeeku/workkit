...
import type { ActionDescriptor, PolicyDefinition, ApprovalGateConfig } from "../src/types";
...
function createMockQueue() {
  const messages: any[] = [];
  return {
    send: vi.fn(async (msg: any) => { messages.push(msg); }),
    _messages: messages,
  } as any;
}
...
describe("createGuard", () => {
  it("returns allowed when no policy matches", async () => {
    const guard = createGuard({
      policies: new Map([
        ["staging", { match: { type: "tag", allOf: ["staging"] }, approvers: ["bob"] } as PolicyDefinition],
      ]),
  ...
  it("creates DO instance for pending request", async () => {
    ...
    const guard = createGuard({
      policies: new Map([
        ["test", { match: { type: "name", pattern: "*" }, approvers: ["bob"], timeout: "1h" } as PolicyDefinition],
      ]),
      storage: doNamespace,
      notificationQueue: createMockQueue(),
  ...
  it("enqueues notifications for each approver", async () => {
    ...
    const guard = createGuard({
      policies: new Map([
        ["test", { match: { type: "name", pattern: "*" }, approvers: ["bob", "carol"], timeout: "1h" } as PolicyDefinition],
      ]),
      storage: createMockDONamespace(),
