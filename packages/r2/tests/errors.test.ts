...
describe('wrapR2Error()', () => {
  ...
  it('wraps 503 as ServiceUnavailableError', () => {
    expect(() =>
      wrapR2Error(new Error('503 service unavailable'), { operation: 'put' }),
    ).toThrow(ServiceUnavailableError)
  })

  it('wraps service unavailable message', () => {
    expect(() =>
      wrapR2Error(new Error('service is unavailable'), { operation: 'list' }),
    ).toThrow(ServiceUnavailableError)
  })
  ...
  it('wraps 404 as NotFoundError', () => {
    expect(() =>
      wrapR2Error(new Error('404'), { operation: 'get', key: 'missing' }),
    ).toThrow(NotFoundError)
  })
  ...
  it('wraps non-Error values', () => {
    expect(() =>
      wrapR2Error('string error', { operation: 'put' }),
    ).toThrow(InternalError)
  })
