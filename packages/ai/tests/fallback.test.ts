...
function createSuccessBinding(
	response: unknown = { response: "OK" },
): AiBinding & { calls: CallRecord[] } {
...
describe("fallback()", () => {
	describe("binding validation", () => {
		it("throws BindingNotFoundError when binding is null", async () => {
			await expect(fallback(null as unknown as AiBinding, [{ model: "a" }], {})).rejects.toThrow(
				"AI",
			);
		});
	...
	describe("empty model list", () => {
		it("throws ServiceUnavailableError with no models", async () => {
			...
			await expect(fallback(binding, [], {})).rejects.toThrow("no models provided");
		});
	});

	describe("first model succeeds", () => {
		it("returns the first model result", async () => {
			const binding = createSequenceBinding([{ result: { response: "From model A" } }]);

			const result = await fallback(
				binding,
				[{ model: "@cf/meta/llama-3.1-70b-instruct" }, { model: "@cf/meta/llama-3.1-8b-instruct" }],
				{
		...
		it("only calls the first model", async () => {
			...
			await fallback(binding, [{ model: "model-a" }, { model: "model-b" }], {});
	...
	describe("first fails, second succeeds", () => {
		it("falls back to the second model", async () => {
			...
			const result = await fallback(binding, [{ model: "model-a" }, { model: "model-b" }], {
				messages: [{ role: "user", content: "Hello" }],
			});
		...
		it("calls both models in order", async () => {
			const binding = createSequenceBinding([{ error: new Error("fail") }, { result: "ok" }]);

			await fallback(binding, [{ model: "first" }, { model: "second" }], {});
	...
	describe("all models fail", () => {
		it("throws ServiceUnavailableError", async () => {
			...
			await expect(
				fallback(binding, [{ model: "a" }, { model: "b" }, { model: "c" }], {}),
		...
		it("includes model names in the error message", async () => {
			...
			await expect(
				fallback(binding, [{ model: "model-x" }, { model: "model-y" }], {}),
	...
	describe("onFallback callback", () => {
		it("calls onFallback when a model fails and there is a next model", async () => {
			...
			await fallback(binding, [{ model: "model-a" }, { model: "model-b" }], {}, { onFallback });

			expect(onFallback).toHaveBeenCalledOnce();
			expect(onFallback).toHaveBeenCalledWith("model-a", expect.any(Error), "model-b");
		});

		it("does not call onFallback when the last model fails", async () => {
			const binding = createSequenceBinding([{ error: new Error("fail") }]);
		...
		it("calls onFallback for each intermediate failure", async () => {
			...
			await fallback(binding, [{ model: "a" }, { model: "b" }, { model: "c" }], {}, { onFallback });
	...
	describe("three-model chain", () => {
		it("skips first two failures and uses third", async () => {
			...
			const result = await fallback(binding, [{ model: "a" }, { model: "b" }, { model: "c" }], {});
	...
	describe("single model chain", () => {
		it("succeeds with single model", async () => {
			const binding = createSequenceBinding([{ result: { response: "Solo" } }]);
		...
		it("fails with single model", async () => {
			const binding = createSequenceBinding([{ error: new Error("Solo failed") }]);

			await expect(fallback(binding, [{ model: "solo" }], {})).rejects.toThrow(
	...
	describe("input passing", () => {
		it("passes the same inputs to each model", async () => {
			...
			await fallback(binding, [{ model: "a" }, { model: "b" }], inputs);
