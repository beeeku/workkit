...
import { withRetry, calculateDelay, defaultIsRetryable } from '../src/retry'
import { TimeoutError, RateLimitError, ServiceUnavailableError, ValidationError } from '@workkit/errors'
...
describe('withRetry()', () => {
  ...
  describe('binding validation', () => {
    it('throws BindingNotFoundError when binding is null', async () => {
      await expect(
        withRetry(null as unknown as AiBinding, 'model', {}),
      ).rejects.toThrow('AI')
    })

    it('throws BindingNotFoundError when binding is undefined', async () => {
      await expect(
        withRetry(undefined as unknown as AiBinding, 'model', {}),
      ).rejects.toThrow('AI')
    })
  })
  ...
  describe('success after retry', () => {
    it('retries on retryable error and succeeds', async () => {
      ...
      const promise = withRetry(binding, 'model', {}, {
        maxRetries: 3,
        backoff: 'fixed',
        baseDelay: 100,
      })
  ...
  describe('default options', () => {
    it('uses default maxRetries of 3', async () => {
      ...
      await expect(
        withRetry(binding, 'model', {}, { baseDelay: 1 }),
      ).rejects.toThrow()
