...
| `createToolRegistry()` | `createToolRegistry()` — re-exported from `@workkit/ai-gateway` and structurally identical. `@workkit/ai`'s export now delegates to the gateway. Only migration step is the tool-type rename: `ToolDefinition` → `GatewayToolDefinition` and `ToolCall` → `GatewayToolCall`. |
