...
describe('compileWhere', () => {
  ...
  it('compiles IN clause: ["status", "IN", ["active", "pending"]]', () => {
    const result = compileWhere(['status', 'IN', ['active', 'pending']] as [string, 'IN', unknown[]])
    expect(result.sql).toBe('status IN (?, ?)')
    expect(result.params).toEqual(['active', 'pending'])
  })

  it('compiles NOT IN clause', () => {
    const result = compileWhere(['status', 'NOT IN', ['deleted', 'banned']] as [string, 'NOT IN', unknown[]])
  ...
  it('compiles BETWEEN clause: ["age", "BETWEEN", [18, 65]]', () => {
    const result = compileWhere(['age', 'BETWEEN', [18, 65]] as [string, 'BETWEEN', [number, number]])
...
describe('SelectBuilder', () => {
  ...
  it('generates WHERE IN clause', () => {
    const builder = new SelectBuilderImpl<User>(db, 'users')
      .where(['id', 'IN', [1, 2, 3]] as [string, 'IN', unknown[]])
    const { sql, params } = builder.toSQL()
  ...
  it('generates WHERE BETWEEN clause', () => {
    const builder = new SelectBuilderImpl<User>(db, 'users')
      .where(['age', 'BETWEEN', [20, 40]] as [string, 'BETWEEN', [number, number]])
    const { sql, params } = builder.toSQL()
  ...
  it('generates WHERE IS NOT NULL', () => {
    const builder = new SelectBuilderImpl<User>(db, 'users')
      .where(['email', 'IS NOT NULL'] as [string, 'IS NOT NULL'])
    const { sql } = builder.toSQL()
