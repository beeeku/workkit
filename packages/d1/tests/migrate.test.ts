...
describe("migrate", () => {
	...
	it("applies pending migrations in order", async () => {
		const migrations = [
			...
			},
			{
				...
				sql: "CREATE TABLE IF NOT EXISTS posts (id INTEGER PRIMARY KEY, title TEXT)",
			},
		];
	...
	it("skips already-applied migrations", async () => {
		const migrations = [
			...
			{
				name: "002_create_posts",
				sql: "CREATE TABLE IF NOT EXISTS posts (id INTEGER PRIMARY KEY)",
			},
		];
		...
		const migrations2 = [
			...migrations,
			{
				...
				sql: "CREATE TABLE IF NOT EXISTS comments (id INTEGER PRIMARY KEY)",
			},
		];
	...
	it("throws D1MigrationError on failure", async () => {
		...
		const migrations = [{ name: "001_bad_sql", sql: "THIS IS NOT VALID SQL" }];
...
describe("validateTableName (SQL injection prevention)", () => {
	...
	it("rejects table names with spaces", async () => {
		await expect(migrate(db, [], { tableName: "my table" })).rejects.toThrow(
			/Invalid migration table name/,
		);
	});

	it("rejects table names starting with numbers", async () => {
		await expect(migrate(db, [], { tableName: "123migrations" })).rejects.toThrow(
			/Invalid migration table name/,
		);
	});

	it("rejects table names with special characters", async () => {
		await expect(migrate(db, [], { tableName: "my-migrations" })).rejects.toThrow(
			/Invalid migration table name/,
		);
	});
