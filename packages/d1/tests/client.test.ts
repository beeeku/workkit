...
import { D1ConstraintError, D1QueryError } from "../src/errors";
...
describe("first()", () => {
	...
	it("wraps D1 errors as D1QueryError", async () => {
		...
		await expect(failDb.first("SELECT * FROM widgets WHERE id = ?", [1])).rejects.toThrow(
			D1QueryError,
		);
	});
});
...
describe("run()", () => {
	...
	it("executes UPDATE and returns changes count", async () => {
		const result = await db.run("UPDATE users SET name = ? WHERE id = ?", ["Updated", 1]);
		expect(result.success).toBe(true);
		expect(result.meta.changes).toBe(1);
	});
