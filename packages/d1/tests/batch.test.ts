...
describe('executeBatch', () => {
  ...
  it('executes multiple statements atomically', async () => {
    const stmt1 = db.prepare('INSERT INTO users (name, email) VALUES (?, ?)').bind('Alice', 'a@test.com')
    const stmt2 = db.prepare('INSERT INTO users (name, email) VALUES (?, ?)').bind('Bob', 'b@test.com')
  ...
  it('returns results for each statement', async () => {
    ...
    const results = await executeBatch(db, [
      { statement: stmt as unknown as D1PreparedStatement },
    ])
