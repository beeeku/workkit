...
describe('createRouter', () => {
  describe('routing', () => {
    ...
    it('extracts multiple path parameters', async () => {
      ...
      const res = await router.fetch(
        makeRequest('GET', '/users/u1/posts/p2'),
        {},
      )
  ...
  describe('validation errors', () => {
    ...
    it('returns 400 for invalid query parameters', async () => {
      ...
      const res = await router.fetch(
        makeRequest('GET', '/users?limit=abc'),
        {},
      )
  ...
  describe('handler returning Response', () => {
    it('passes through Response objects from handler', async () => {
      const endpoint = api({
        method: 'GET',
        path: '/custom',
        handler: async () => new Response('raw', {
          status: 201,
  ...
  describe('routes property', () => {
    it('exposes registered routes', () => {
      ...
      const b = api({ method: 'POST', path: '/b', body: z.object({ x: z.number() }), handler: async () => 'b' })
