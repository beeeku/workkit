...
export type PathParamRecord<Path extends string> = [ExtractPathParams<Path>] extends [never]
	? Record<string, never>
...
type CleanPathName<S extends string> = S extends `${infer Part}/${infer Rest}`
	? Part extends `:${string}`
		? CleanPathName<Rest>
...
>
	? (TParams extends StandardSchemaV1
			...
			: ExtractPathParams<TPath & string> extends never
				? {}
				: { params: PathParamRecord<TPath & string> }) &
