...
export function createClient(config: ClientConfig & { apis?: ApiDefinition[] }): ApiClient {
  ...
  return {
    async call<TDef extends ApiDefinition>(
      definition: TDef,
      input?: CallInput,
    ): Promise<unknown> {
      // Build the URL
      const path = input?.params
        ? buildPath(definition.path, input.params)
        : definition.path
      ...
      if (input?.body !== undefined && definition.method !== 'GET' && definition.method !== 'HEAD') {
     ...
     */
    for<TDef extends ApiDefinition>(
      definition: TDef,
    ): (input?: CallInput) => Promise<unknown> {
...
export interface ApiClient {
  call<TDef extends ApiDefinition>(
    definition: TDef,
    input?: CallInput,
  ): Promise<unknown>
  for<TDef extends ApiDefinition>(
