...
describe("wrapKVError", () => {
	it("wraps timeout errors as TimeoutError", () => {
		expect(() => wrapKVError(new Error("operation timed out"), { operation: "get" })).toThrow(
			TimeoutError,
		);
	});

	it("wraps service errors as ServiceUnavailableError", () => {
		expect(() => wrapKVError(new Error("503 service unavailable"), { operation: "get" })).toThrow(
			ServiceUnavailableError,
		);
	});

	it("wraps unknown errors as InternalError", () => {
		expect(() => wrapKVError(new Error("something weird"), { operation: "get" })).toThrow(
			InternalError,
		);
	});
