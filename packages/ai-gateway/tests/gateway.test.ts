...
import { ConfigError, ValidationError, ServiceUnavailableError } from '@workkit/errors'
import type { AiInput, AiOutput, Gateway, CustomProviderConfig } from '../src/types'
...
describe('createGateway()', () => {
  it('throws ConfigError with no providers', () => {
    expect(() =>
      createGateway({ providers: {}, defaultProvider: 'none' }),
    ).toThrow(ConfigError)
  })
...
describe('run() — provider override', () => {
  ...
  it('throws ConfigError for unknown provider override', async () => {
    ...
    await expect(
      gw.run('model', { prompt: 'test' }, { provider: 'nonexistent' }),
