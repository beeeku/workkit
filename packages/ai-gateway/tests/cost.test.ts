...
describe('record()', () => {
  ...
  it('throws ConfigError for negative input tokens', () => {
    expect(() =>
      tracker.record('gpt-4', { inputTokens: -100, outputTokens: 200 }),
    ).toThrow(ConfigError)
  })
...
describe('checkBudget()', () => {
  it('returns full budget when no usage', () => {
    ...
    const budget = tracker.checkBudget(1.00)
    expect(budget.remaining).toBeCloseTo(1.00)
  ...
  it('reports exceeded when over budget', () => {
    ...
    // 0.3 + 0.6 = 0.9
    const budget = tracker.checkBudget(0.50)
