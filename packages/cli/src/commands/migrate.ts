 ...
 */
export async function discoverMigrations(dir: string, fs: FileSystem): Promise<MigrationFile[]> {
...
export function validateMigrationName(name: string): { valid: boolean; reason?: string } {
	...
	if (!/^\d+_/.test(name)) {
		return {
			valid: false,
			reason: `Migration "${name}" should start with a numeric prefix (e.g., 001_description)`,
		};
	}
