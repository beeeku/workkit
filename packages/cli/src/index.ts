import { parseArgs, createNodeFs, VERSION, log, error, bold, cyan, dim } from './utils'
import { executeInit, parseFeatures, VALID_TEMPLATES, VALID_FEATURES } from './commands/init'
...
import type { Template, Feature } from './commands/init'
    ...
    switch (command) {
      ...
      case 'd1': {
        if (subcommand === 'migrate') {
          ...
          log(`Generated ${result.statements.length} INSERT statements for table "${result.table}"`)
      ...
      case 'gen': {
        if (subcommand === 'client') {
          ...
          const output = (flags['output'] as string) ?? (flags['o'] as string) ?? './src/generated/client.ts'
