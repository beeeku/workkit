...
- 2f2665e: **`workkit init` now pins `@workkit/*` versions instead of `"latest"`.** Previously the scaffolded `package.json` used `"latest"` for every workkit package, which made installs non-reproducible (each `bun install` could resolve a different version) and silently pulled breaking releases into existing projects.
