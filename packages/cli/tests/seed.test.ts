...
describe('seed command', () => {
  ...
  describe('generateInsertStatements', () => {
    ...
    it('throws for invalid table name', () => {
      expect(() => generateInsertStatements('drop table;--', [{ a: 1 }])).toThrow('Invalid table name')
    })
  })
  ...
  describe('executeSeed', () => {
    ...
    it('throws for missing file', async () => {
      const fs = createMockFs()
      await expect(
        executeSeed({ file: '/missing.json', table: 'users' }, fs),
    ...
    it('respects explicit format override', async () => {
      ...
      const result = await executeSeed(
        { file: '/seeds/data.txt', table: 't', format: 'json' },
        fs,
      )
