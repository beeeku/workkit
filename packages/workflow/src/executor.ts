import type { StepDefinition, WorkflowConfig, StepContext, WorkflowError, ExecutionMeta, Logger } from "./types";
  ...
  return {
    async execute(
      ...
      for (let i = startIndex; i < steps.length; i++) {
        ...
        const retryConfig = step.options?.retry ?? config.retry ?? {
          maxAttempts: 1, initialDelay: 0, maxDelay: 0, backoffMultiplier: 1,
        };
        ...
        for (let attempt = 1; attempt <= retryConfig.maxAttempts; attempt++) {
          ...
          try {
            ...
            await journal.writeEntry(i, {
              ...
              input: Object.keys(prev),
              error: { name: error.name, message: error.message, retryable: attempt < retryConfig.maxAttempts },
              attempt,
              startedAt: Date.now(),
            });
