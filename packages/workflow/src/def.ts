...
export function createWorkflowDef<TInput, TOutput extends Record<string, unknown>>(
	name: string,
	steps: StepDefinition[],
	config: WorkflowConfig,
	_compensation?: CompensationHandler<TInput, TOutput>,
	...
	function createHandle(executionId: string): ExecutionHandle<TOutput> {
		return {
			...
			async status(): Promise<any> {
				if (config.backend.type !== "do") throw new Error("Only DO backend supported in v0.1.0");
			...
			async result(): Promise<any> {
				...
				if (currentStatus === "completed") return { ok: true, value: {} as TOutput };
			...
			async cancel() {
				...
				const doId = config.backend.namespace.idFromName(executionId);
				const stub = config.backend.namespace.get(doId);
				await stub.fetch(new Request("https://internal/cancel", { method: "POST" }));
			},
		};
	}
	...
	return {
		...
		async run(input: TInput, options?: RunOptions): Promise<ExecutionHandle<TOutput>> {
			if (config.backend.type !== "do") throw new Error("Only DO backend supported in v0.1.0");
