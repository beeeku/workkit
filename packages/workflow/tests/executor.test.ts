...
import type { StepDefinition, RetryStrategy } from "../src/types";
...
describe("createExecutor", () => {
  it("executes steps in order", async () => {
    ...
    const steps: StepDefinition[] = [
      { name: "a", type: "step", handler: async () => { order.push("a"); return { a: 1 }; } },
      { name: "b", type: "step", handler: async () => { order.push("b"); return { b: 2 }; } },
      { name: "c", type: "step", handler: async () => { order.push("c"); return { c: 3 }; } },
    ];
  ...
  it("passes input and prev to each step", async () => {
    ...
    const steps: StepDefinition[] = [
      { name: "first", type: "step", handler: async (input: any, prev: any) => {
        received.push({ input, prev });
        return { value: 10 };
      }},
      { name: "second", type: "step", handler: async (input: any, prev: any) => {
        received.push({ input, prev });
  ...
  it("handles step failure", async () => {
    ...
    const steps: StepDefinition[] = [
      ...
      { name: "fail", type: "step", handler: async () => { throw new Error("boom"); } },
    ];
  ...
  it("skips completed steps on replay", async () => {
    ...
    await journal.writeEntry(0, {
      index: 0, name: "a", status: "completed",
      input: {}, output: { cached: true }, attempt: 1,
    });
    ...
    const steps: StepDefinition[] = [
      { name: "a", type: "step", handler: async () => { called.push("a"); return { cached: true }; } },
      { name: "b", type: "step", handler: async () => { called.push("b"); return { fresh: true }; } },
    ];
  ...
  it("retries failed steps", async () => {
    ...
    const steps: StepDefinition[] = [
      { name: "flaky", type: "step", handler: async () => {
        attempts++;
