...
import { createMockD1, createFailingD1 } from '../src/d1'
...
describe('createMockD1', () => {
  ...
  describe('CREATE TABLE', () => {
    it('creates a table', async () => {
      const result = await db.prepare('CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT)').run()
  ...
  describe('INSERT', () => {
    ...
    it('inserts a row', async () => {
      const result = await db.prepare('INSERT INTO users (name, email) VALUES (?, ?)').bind('Alice', 'alice@test.com').run()
      expect(result.success).toBe(true)
      expect(result.meta.changes).toBe(1)
    })

    it('returns inserted row with RETURNING', async () => {
      const result = await db.prepare('INSERT INTO users (name, email) VALUES (?, ?) RETURNING *').bind('Bob', 'bob@test.com').all()
  ...
  describe('SELECT', () => {
    ...
    it('filters with WHERE IN', async () => {
      const result = await db.prepare('SELECT * FROM users WHERE name IN (?, ?)').bind('Alice', 'Charlie').all()
      expect(result.results).toHaveLength(2)
    })

    it('filters with WHERE NOT IN', async () => {
      const result = await db.prepare('SELECT * FROM users WHERE name NOT IN (?)').bind('Bob').all()
      expect(result.results).toHaveLength(2)
    })

    it('filters with WHERE BETWEEN', async () => {
      const result = await db.prepare('SELECT * FROM users WHERE age BETWEEN ? AND ?').bind(25, 30).all()
      expect(result.results).toHaveLength(2)
    })

    it('filters with WHERE IS NULL', async () => {
      db = createMockD1({ users: [{ id: 1, name: 'Alice', email: null }, { id: 2, name: 'Bob', email: 'bob@test.com' }] })
  ...
  describe('UPDATE', () => {
    beforeEach(async () => {
      db = createMockD1({ users: [{ id: 1, name: 'Alice', age: 30 }, { id: 2, name: 'Bob', age: 25 }] })
    })

    it('updates matching rows', async () => {
      const result = await db.prepare('UPDATE users SET age = ? WHERE name = ?').bind(31, 'Alice').run()
      expect(result.meta.changes).toBe(1)
      const row = await db.prepare('SELECT * FROM users WHERE name = ?').bind('Alice').first()
    ...
    it('returns updated rows with RETURNING', async () => {
      const result = await db.prepare('UPDATE users SET age = ? WHERE name = ? RETURNING *').bind(31, 'Alice').all()
      expect(result.results).toHaveLength(1)
  ...
  describe('DELETE', () => {
    beforeEach(async () => {
      db = createMockD1({ users: [{ id: 1, name: 'Alice' }, { id: 2, name: 'Bob' }, { id: 3, name: 'Charlie' }] })
    })
    ...
    it('returns deleted rows with RETURNING', async () => {
      const result = await db.prepare('DELETE FROM users WHERE name = ? RETURNING *').bind('Bob').all()
      expect(result.results).toHaveLength(1)
      expect(result.results[0].name).toBe('Bob')
    })
  })
