...
export function createMockD1(
  ...
  function executeUpdate(
    ...
    const setCols = setMatch[1].split(',').map((s) => s.trim().split('=')[0].trim().replace(/["`]/g, ''))
  ...
  function executeDelete(
    ...
    if (/RETURNING/i.test(sql)) {
      return {
        results: deleted,
        meta: mockMeta({ changed_db: deleted.length > 0, changes: deleted.length, rows_written: deleted.length }),
      }
    }
  ...
  function applyWhere(
    ...
    const whereMatch = sql.match(/WHERE\s+(.*?)(?:\s+ORDER|\s+LIMIT|\s+GROUP|\s+HAVING|\s+RETURNING|$)/i)
    ...
    return rows.filter((row) => {
      ...
      for (const rc of resolvedConditions) {
        ...
        if (rc.isNull) {
          ...
          match = rc.operator === 'NOT IN'
            ? !rc.values.includes(row[rc.column])
            : rc.values.includes(row[rc.column])
        } else if (rc.betweenFlag) {
          match = (row[rc.column] as any) >= (rc.values[0] as any) && (row[rc.column] as any) <= (rc.values[1] as any)
        } else {
          const val = rc.values[0]
          switch (rc.operator) {
            case '=': match = row[rc.column] == val; break
            case '!=': match = row[rc.column] != val; break
            case '>': match = (row[rc.column] as any) > (val as any); break
            case '<': match = (row[rc.column] as any) < (val as any); break
            case '>=': match = (row[rc.column] as any) >= (val as any); break
            case '<=': match = (row[rc.column] as any) <= (val as any); break
            case 'LIKE': {
            ...
            default: match = row[rc.column] === val
          }
        }
  ...
  function parseWhereConditions(whereClause: string): ParsedCondition[] {
    ...
    // Restore IN clauses
    clause = clause.replace(/__IN_PLACEHOLDER_(\d+)__/g, (_, idx) => inClauses[parseInt(idx)])
    const tokens = clause.split(/\s+(AND|OR)\s+/i)
    // Restore BETWEEN clauses in each token
    for (let i = 0; i < tokens.length; i++) {
      tokens[i] = tokens[i].replace(/__BETWEEN_PLACEHOLDER_(\d+)__/g, (_, idx) => betweenClauses[parseInt(idx)])
    }
    ...
    for (const part of parts) {
      ...
      if (isNotNullMatch) {
        conditions.push({ column: isNotNullMatch[1], operator: 'IS NOT NULL', joiner: part.joiner, isNull: false, isNotNull: true })
        continue
      }

      const isNullMatch = sql.match(/["`]?(\w+)["`]?\s+IS\s+NULL/i)
      if (isNullMatch) {
        conditions.push({ column: isNullMatch[1], operator: 'IS NULL', joiner: part.joiner, isNull: true, isNotNull: false })
        continue
      }
...
export function createFailingD1(error: Error | string): D1Database {
  ...
  return {
    prepare: () => failStmt,
    batch: async () => { throw err },
    exec: async () => { throw err },
    dump: async () => { throw err },
