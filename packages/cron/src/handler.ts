...
import type { CronHandler, CronHandlerOptions, CronTaskHandler, CronMiddleware } from './types'
  ...
  return async (
    ...
    for (const [name, task] of Object.entries(tasks)) {
      if (matchCron(task.schedule, event.cron)) {
        const wrappedHandler = middleware.length > 0
          ? applyMiddleware(task.handler, middleware, name)
          : task.handler
