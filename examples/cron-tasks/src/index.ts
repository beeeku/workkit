import { createCronHandler, withLock, withRetry, withTimeout } from "@workkit/cron";
...
async function cleanupSessions(
	...
	try {
		...
		const result = await db.run("DELETE FROM sessions WHERE expires_at < ?", [cutoff]);
...
async function generateDailyReport(
	...
	const [userCount, newUsers, activeUsers] = await Promise.all([
		...
		db.first<{ count: number }>("SELECT COUNT(*) as count FROM users WHERE created_at >= ?", [
			yesterday,
		]),
	...
	console.log(
		`[daily-report] Generated report for ${today}: ${userCount} total, ${newUsers?.count ?? 0} new, ${activeUsers?.count ?? 0} active`,
	);
}
...
export default {
	...
	async fetch(request: Request, rawEnv: Record<string, unknown>): Promise<Response> {
		...
		if (url.pathname === "/status") {
			...
			const runs = await db.all<JobRun>("SELECT * FROM job_runs ORDER BY started_at DESC LIMIT 20");
