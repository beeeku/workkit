...
app.post("/api/auth/register", async (c) => {
	...
	const existing = await db.first<User>("SELECT id FROM users WHERE email = ?", [body.email]);
	if (existing) {
...
app.post("/api/auth/login", async (c) => {
	...
	const user = await db.first<User>("SELECT * FROM users WHERE email = ?", [body.email]);
...
app.get("/api/tasks", async (c) => {
	...
	let query = db.select<Task>("tasks").where({ user_id: Number.parseInt(auth.sub, 10) });
	...
	const countResult = await swr<{ total: number }>({
		...
		async fetch() {
			let countQuery = db.select<Task>("tasks").where({ user_id: Number.parseInt(auth.sub, 10) });
...
app.get("/api/tasks/:id", async (c) => {
	...
	const task = await db
		.select<Task>("tasks")
		.where({ id: Number.parseInt(c.req.param("id"), 10), user_id: Number.parseInt(auth.sub, 10) })
		.first();
...
app.post("/api/tasks", async (c) => {
	...
	const task = await db
		.insert("tasks")
		.values({
			user_id: Number.parseInt(auth.sub, 10),
			title: body.title,
...
app.get("/api/stats", async (c) => {
	...
	const env = getEnv(c);
	const db = d1(env.DB, { transformColumns: "camelCase" });
	const userId = Number.parseInt(auth.sub, 10);
