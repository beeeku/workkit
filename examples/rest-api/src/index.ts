...
app.get("/users/:id", async (c) => {
	...
	const user = await db
		.select<User>("users")
		.where({ id: Number.parseInt(c.req.param("id"), 10) })
		.first();
...
app.patch("/users/:id", async (c) => {
	...
	const updated = await db.update("users").set(body).where({ id }).returning<User>("*").first();
...
app.delete("/users/:id", async (c) => {
	...
	const deleted = await db.delete("users").where({ id }).returning<User>("id").first();
