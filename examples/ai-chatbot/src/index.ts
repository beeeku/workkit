...
import { slidingWindow, rateLimitHeaders, rateLimitResponse } from '@workkit/ratelimit'
import { swr, cacheAside } from '@workkit/cache'
import { z } from 'zod'
...
app.use('/chat/*', async (c, next) => {
  ...
  if (!result.allowed) {
    return rateLimitResponse(result, 'Rate limit exceeded. Please wait before sending more messages.')
  }
...
app.post('/chat/stream', async (c) => {
  ...
  const stream = await streamAI(env.AI, body.model, {
    messages: [
      { role: 'system', content: body.systemPrompt },
      { role: 'user', content: body.message },
    ],
...
app.post('/chat/faq', async (c) => {
  ...
  const result = await swr<{ response: string }>({
    ...
    async fetch() {
      const client = ai(env.AI)
      const aiResult = await client.run<{ response: string }>(
        '@cf/meta/llama-3.1-8b-instruct',
        {
          messages: [
            { role: 'system', content: 'Answer frequently asked questions concisely. Keep answers under 200 words.' },
