...
import { createTestEnv, createMockKV, createMockD1 } from './helpers/setup'
...
describe('KV + D1 cross-package flow', () => {
  it('stores data in D1 and caches in KV', async () => {
    ...
    await db.exec('CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT, email TEXT, active INTEGER)')
    await db.run('INSERT INTO users (name, email, active) VALUES (?, ?, ?)', ['Alice', 'alice@example.com', 1])
  ...
  it('uses KV as a write-through cache for D1 data', async () => {
    ...
    await db.run('INSERT INTO users (name, email, active) VALUES (?, ?, ?)', ['Alice', 'alice@test.com', 1])
    await db.run('INSERT INTO users (name, email, active) VALUES (?, ?, ?)', ['Bob', 'bob@test.com', 1])
    await db.run('INSERT INTO users (name, email, active) VALUES (?, ?, ?)', ['Charlie', 'charlie@test.com', 0])
  ...
  it('uses D1 query builder with KV caching', async () => {
    ...
    await db.exec('CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT, email TEXT, active INTEGER)')
    await db.run('INSERT INTO users (name, email, active) VALUES (?, ?, ?)', ['Alice', 'alice@test.com', 1])
    await db.run('INSERT INTO users (name, email, active) VALUES (?, ?, ?)', ['Bob', 'bob@test.com', 0])
  ...
  it('handles batch operations across KV and D1', async () => {
    ...
    await db.run('INSERT INTO users (name, email, active) VALUES (?, ?, ?)', ['Alice', 'a@test.com', 1])
    await db.run('INSERT INTO users (name, email, active) VALUES (?, ?, ?)', ['Bob', 'b@test.com', 1])
    await db.run('INSERT INTO users (name, email, active) VALUES (?, ?, ?)', ['Charlie', 'c@test.com', 1])
  ...
  it('D1 camelCase transform works with KV storage', async () => {
    ...
    const cache = kv<{ id: number; fullName: string; isActive: boolean }>(env.CACHE, { prefix: 'user:' })
    ...
    const user = await db.first<{ id: number; fullName: string; isActive: number }>('SELECT * FROM users WHERE id = ?', [1])
  ...
  it('lists KV entries that were populated from D1', async () => {
    ...
    await db.exec('CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT, email TEXT, active INTEGER)')
    for (let i = 1; i <= 5; i++) {
      await db.run('INSERT INTO users (name, email, active) VALUES (?, ?, ?)', [`User${i}`, `user${i}@test.com`, 1])
    }
