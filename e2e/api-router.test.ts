...
import { createStringSchema, createNumberSchema, createObjectSchema } from './helpers/setup'
...
describe('API router E2E', () => {
  describe('define APIs and route requests', () => {
    ...
    it('supports PUT and DELETE methods', async () => {
      ...
      const delRes = await router.fetch(
        createRequest('/users/1', { method: 'DELETE' }),
        {},
      )
  ...
  describe('router features', () => {
    ...
    it('handler receives env', async () => {
      ...
      const res = await router.fetch(createRequest('/config'), { API_URL: 'https://api.example.com' })
  ...
  describe('api definition validation', () => {
    it('throws if method is missing', () => {
      expect(() =>
        api({ method: '' as any, path: '/test', handler: async () => ({}) }),
      ).toThrow('requires a method')
    })

    it('throws if path is missing', () => {
      expect(() =>
        api({ method: 'GET', path: '' as any, handler: async () => ({}) }),
      ).toThrow('requires a path')
    })

    it('throws if path does not start with /', () => {
      expect(() =>
        api({ method: 'GET', path: 'no-slash', handler: async () => ({}) }),
      ).toThrow("start with '/'")
    })

    it('throws if handler is missing', () => {
      expect(() =>
        api({ method: 'GET', path: '/test', handler: undefined as any }),
