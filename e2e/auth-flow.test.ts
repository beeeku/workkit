...
describe("Auth flow E2E", () => {
	describe("JWT sign and verify", () => {
		...
		it("rejects expired JWT", async () => {
			const shortToken = await signJWT({ userId: "123" }, { secret: JWT_SECRET, expiresIn: "1s" });
			...
			await expect(verifyJWT(shortToken, { secret: JWT_SECRET })).rejects.toThrow("expired");
		});

		it("rejects JWT with wrong secret", async () => {
			const token = await signJWT({ userId: "123" }, { secret: JWT_SECRET });

			await expect(verifyJWT(token, { secret: "wrong-secret" })).rejects.toThrow("signature");
		});

		it("validates issuer claim", async () => {
			const token = await signJWT({ userId: "123" }, { secret: JWT_SECRET, issuer: "my-app" });
			...
			// Wrong issuer
			await expect(verifyJWT(token, { secret: JWT_SECRET, issuer: "other-app" })).rejects.toThrow(
				"issuer",
			);
		});

		it("validates audience claim", async () => {
			const token = await signJWT({ userId: "123" }, { secret: JWT_SECRET, audience: "api" });
			...
			await expect(verifyJWT(token, { secret: JWT_SECRET, audience: "web" })).rejects.toThrow(
				"audience",
			);
		});
		...
		it("rejects disallowed algorithm", async () => {
			...
			await expect(verifyJWT(token, { secret: JWT_SECRET, algorithms: ["HS512"] })).rejects.toThrow(
				"not allowed",
			);
		});

		it("supports HS384 algorithm", async () => {
			const token = await signJWT({ data: "test" }, { secret: JWT_SECRET, algorithm: "HS384" });
		...
		it("supports HS512 algorithm", async () => {
			const token = await signJWT({ data: "test" }, { secret: JWT_SECRET, algorithm: "HS512" });
